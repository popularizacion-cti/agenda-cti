...
function renderWeeklyAgenda(list) {
  ...
  if (!semanal.length) {
    cont.innerHTML = `<div class="sin-actividades">No hay actividades para el resto de la semana.</div>`;
    return;
  }
  semanal.forEach(a => cont.appendChild(buildCard(a)));
}
...
function buildFilters(list) {
  ...
  div.innerHTML = `
    ...
    <div style="margin-top:10px;">
      Desde: <input type="date" id="f-desde"> 
      Hasta: <input type="date" id="f-hasta">
      <button class="btn" onclick="applyFilters()" style="padding: 8px 20px; margin-left:10px;">Filtrar</button>
      <button class="btn btn-alt" onclick="clearFilters()" style="padding: 8px 20px;">Limpiar</button>
    </div>
  `;
}
...
function buildCard(a) {
  ...
  div.innerHTML = `
    ...
    ${a["Enlace del evento"] ? `<br><a href="${a["Enlace del evento"]}" target="_blank" class="btn" style="padding:5px 15px; font-size:0.8em;">Ver Evento</a>` : ''}
  `;
  return div;
}
