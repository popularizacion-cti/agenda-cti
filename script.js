...
// ... (Mantenemos tus funciones parseFechaFiltro, mostrarFecha, limpiarHora, mostrarHora igual)
...
fetch(ENDPOINT)
  ...
  .then(data => {
    ALL_ACTIVITIES = data
      .filter(a => a["Aprobado"]?.toLowerCase() === "si")
      .map(a => ({
        ...a,
        _fechaFiltro: parseFechaFiltro(a["Fecha de realización"]),
        _publicos: a["Público objetivo"] ? a["Público objetivo"].split(",").map(p => p.trim()) : []
      }));
    renderWeeklyAgenda(ALL_ACTIVITIES);
    buildFilters(ALL_ACTIVITIES);
  })
...
function renderWeeklyAgenda(list) {
  ...
  const semanal = list
    .filter(a => a._fechaFiltro && a._fechaFiltro >= (hoy > lunes ? hoy : lunes) && a._fechaFiltro <= domingo)
    .sort((a, b) => a._fechaFiltro - b._fechaFiltro);

  if (!semanal.length) {
    cont.innerHTML = `<div class="actividad" style="border-left-color: #ffc107; background-color: #fffbeb;">No hay actividades para el resto de la semana.</div>`;
    return;
  }
  semanal.forEach(a => cont.appendChild(buildCard(a)));
}
...
function buildFilters(list) {
  ...
  div.innerHTML = `
    ...
    <div style="margin-top:15px; text-align:center;">
      <span style="color:#666;">Desde:</span> <input type="date" id="f-desde"> 
      <span style="color:#666;">Hasta:</span> <input type="date" id="f-hasta">
      <br><br>
      <button class="btn btn-primary" onclick="applyFilters()" style="padding: 10px 25px; cursor:pointer;">Filtrar</button>
      <button class="btn btn-secondary" onclick="clearFilters()" style="padding: 10px 25px; cursor:pointer; margin-left:10px;">Limpiar</button>
    </div>
  `;
}

function applyFilters() {
  const region = f("f-region"), modalidad = f("f-modalidad"), publico = f("f-publico");
  const dDesde = f("f-desde") ? new Date(f("f-desde") + "T00:00:00") : null;
  const dHasta = f("f-hasta") ? new Date(f("f-hasta") + "T23:59:59.999") : null;
...
function renderFilterResults(list) {
  const div = document.getElementById("resultados-filtro");
  div.innerHTML = "<h2>📋 Resultados del filtro</h2>";
  if (!list.length) { div.innerHTML += `<p>No se encontraron actividades.</p>`; return; }
  list.forEach(a => div.appendChild(buildCard(a)));
}

function clearFilters() {
  document.querySelectorAll("#filtros-cti select, #filtros-cti input").forEach(e => (e.value = ""));
  document.getElementById("resultados-filtro").innerHTML = "";
}
...
function buildCard(a) {
  ...
  div.innerHTML = `
    <div class="fecha">${fecha} ${hora ? " | " + hora : ""}</div>
    <h3>${a["Nombre de la actividad"].toUpperCase()}</h3>
    <div class="institucion">${a["Nombre de la entidad"].toUpperCase()} | ${a["Región"].toUpperCase()}</div>
    <p style="white-space: pre-line; color: #333;">${a["Resumen de la actividad"]}</p>
    <div style="font-size:0.9em; color:#666; margin-top:15px; border-top:1px solid #eee; padding-top:10px;">
      <strong>Público:</strong> ${a["Público objetivo"]} | <strong>Modalidad:</strong> ${a["Modalidad"]}
    </div>
    ${a["Enlace del evento"] ? `<br><a href="${a["Enlace del evento"]}" target="_blank" class="btn btn-primary" style="padding:6px 18px; font-size:0.85em;">Ver Evento</a>` : ''}
  `;
  return div;
}
